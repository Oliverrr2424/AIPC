...
import { getMarketSignals, isPartEligibleForRegion, marketRegion, summarizeBuildMarket, withMarketPrice } from "@/lib/pricing/marketSignals";
import { checkCompatibility, estimateWattage, hasKnownCompatibilityFailure } from "@/lib/compatibility/compatibilityChecker";
...
export async function generateBuild(request:BuildRequest):Promise<BuildRecommendation>{
 const region=marketRegion(request.country),marketSignals=await getMarketSignals(catalog,request.country),marketCatalog=catalog.filter(part=>isPartEligibleForRegion(part,region)&&isCatalogPartSelectable(part)).map(part=>withMarketPrice(part,marketSignals.get(part.id)));
 ...
 const ramPool=rams.filter(x=>x.memoryType===motherboard.memoryType&&(!cpu.memoryTypes?.length||cpu.memoryTypes.includes(x.memoryType))&&x.capacityGb<=motherboard.maxMemoryGb&&(motherboard.ramSlots==null||x.sticks<=motherboard.ramSlots));
 const ram=existing<typeof rams[number]>(request,"ram",rams)||inBudget(ramPool.length?ramPool:rams,budget*a.ram,c,x=>Math.min(x.capacityGb,ramNeed)*2+x.speedMt/500-(x.capacityGb>ramNeed*2?30:0));
 const storagePool=storages.filter(x=>motherboard.storageInterfaces.includes(x.interface));
 const storage=existing<typeof storages[number]>(request,"storage",storages)||inBudget(storagePool.length?storagePool:storages,budget*a.storage,c,x=>x.capacityTb*25+(x.readSpeedMb||0)/500);
 const casePool=cases.filter(x=>x.supportedMotherboardFormFactors.includes(motherboard.formFactor)&&x.maxGpuLengthMm>=gpu.lengthMm&&(gpu.thicknessSlots==null||x.maxGpuThicknessSlots==null||gpu.thicknessSlots<=x.maxGpuThicknessSlots)&&(request.preferSmallFormFactor?x.supportedMotherboardFormFactors.length===1&&x.supportedMotherboardFormFactors[0]==="Mini-ITX":x.supportedMotherboardFormFactors.length>1));
 ...
 const coolerPool=coolers.filter(x=>x.supportedSockets.includes(cpu.socket)&&x.tdpRatingWatts>=cpu.tdpWatts&&(x.type==="aio"?(x.radiatorSizeMm==null||pcCase.supportedRadiatorSizesMm==null||pcCase.supportedRadiatorSizesMm.includes(x.radiatorSizeMm)):!x.heightMm||x.heightMm<=pcCase.maxCoolerHeightMm));
 ...
 const psuPool=psus.filter(x=>x.wattage>=needed&&pcCase.psuFormFactors.includes(x.formFactor)&&!hasKnownCompatibilityFailure({...partial,psu:x}));
 ...
 const ownedIds=new Set(request.existingPartIds||[]);let totalPrice=Object.values(parts).reduce((sum,p)=>sum+(ownedIds.has(p.id)?0:priceIn(p,c)),0);
 // Trim optional capacity before core performance when an estimate exceeds budget.
 if(totalPrice>budget){const cheaperStorage=[...storages].filter(x=>priceIn(x,c)<priceIn(parts.storage,c)).sort((x,y)=>priceIn(y,c)-priceIn(x,c))[0]; if(cheaperStorage){parts.storage=cheaperStorage; totalPrice=Object.values(parts).reduce((s,p)=>s+(ownedIds.has(p.id)?0:priceIn(p,c)),0)}}
 if(totalPrice>budget){const cheaperRam=[...rams].filter(x=>priceIn(x,c)<priceIn(parts.ram,c)).sort((x,y)=>priceIn(y,c)-priceIn(x,c))[0]; if(cheaperRam){parts.ram=cheaperRam; totalPrice=Object.values(parts).reduce((s,p)=>s+(ownedIds.has(p.id)?0:priceIn(p,c)),0)}}
 ...
 const raw={id,title,request,parts,totalPrice,estimatedWattage,compatibility,performance,alternatives,generatedAt:new Date().toISOString(),market:summarizeBuildMarket(Object.values(parts),marketSignals,region)};
