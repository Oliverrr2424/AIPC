import { cases, coolers, cpus, gpus, motherboards, partById, psus, rams, storages } from "@/data/parts";
...
const existing=<T extends Part>(r:BuildRequest,category:T["category"])=>(r.existingPartIds||[]).map(partById).find(p=>p?.category===category) as T|undefined;
export async function generateBuild(request:BuildRequest):Promise<BuildRecommendation>{
 ...
 const gpu=existing<typeof gpus[number]>(request,"gpu")||inBudget(gpus,budget*a.gpu,c,gpuScore);
 ...
 const cpu=existing<typeof cpus[number]>(request,"cpu")||inBudget(cpus,budget*a.cpu,c,cpuScore);
 ...
 const motherboard=existing<typeof motherboards[number]>(request,"motherboard")||inBudget(boardPool.length?boardPool:motherboards,budget*a.motherboard,c,x=>x.m2Slots*10+(x.cpuTiers.includes(cpu.tier)?30:0)+(request.preferUpgradeability?x.maxMemoryGb/8:0));
 ...
 const psu=existing<typeof psus[number]>(request,"psu")||[...psuPool].sort((x,y)=>x.wattage-y.wattage)[0]||psus[psus.length-1];
 ...
 const raw={id,title,request,parts,totalPrice,estimatedWattage,compatibility,performance,alternatives,generatedAt:new Date().toISOString()};
