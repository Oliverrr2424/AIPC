...
export function AiExplanationPanel({ text }: { text: string }) {
  ...
  return <section className="surface rounded-2xl p-6 sm:p-8">
    <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-[var(--accent-soft)] text-[var(--accent)]"><Sparkle weight="fill"/></span><div><h2 className="text-xl font-semibold">{t("explanation.title")}</h2><p className="text-sm text-[var(--muted)]">{t("explanation.subtitle")}</p></div></div>
