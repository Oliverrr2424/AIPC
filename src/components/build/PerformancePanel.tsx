...
export function PerformancePanel({ build }: { build: BuildRecommendation }) {
  ...
  const p = build.parts;
  const data = [
    { subject: "Gaming", value: p.gpu.gamingScore1440p * .78 + p.cpu.gamingScore * .22 },
    { subject: "AI", value: p.gpu.aiScore },
    { subject: "CPU", value: p.cpu.productivityScore },
    { subject: "Memory", value: Math.min(100, p.ram.capacityGb / 1.28) },
    { subject: "Storage", value: Math.min(100, (p.storage.readSpeedMb || 500) / 75) },
  ];
  ...
  return <section className="surface rounded-2xl p-6">
    ...
    <div className="grid grid-cols-2 gap-3">
      {perf.gaming && <div className="rounded-xl bg-[var(--panel-2)] p-4"><div className="text-xs text-[var(--muted)]">{perf.gaming.resolution} gaming</div><strong className="mt-1 block">{perf.gaming.estimatedFpsTier}</strong></div>}
