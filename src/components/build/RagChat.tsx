...
function RagResult({ result }: { result: RagBuildRecommendation }) {
  ...
  return <div className="mt-10 space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]"><PartsTable build={result}/><div className="space-y-6"><PerformancePanel build={result}/><CompatibilityPanel results={result.compatibility}/></div></div>
...
function RagSkeleton() { return <div className="mt-10 animate-pulse space-y-6"><div className="h-28 rounded-2xl bg-[var(--panel-2)]"/><div className="grid gap-4 lg:grid-cols-2">{[0,1,2,3].map(item => <div key={item} className="h-36 rounded-xl bg-[var(--panel-2)]"/>)}</div></div>; }
